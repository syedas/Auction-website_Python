...
class JSONParser(parsers.JSONParser):
    ...
    def parse(self, stream, media_type=None, parser_context=None):
        ...
        if data:
            ...
            if isinstance(parser_context['view'], RelationshipView):
                ...
                if isinstance(data, list):
                    for resource_identifier_object in data:
                        if not (resource_identifier_object.get('auction') and resource_identifier_object.get('type')):
                            raise ParseError(
                                'Received data contains one or more malformed JSONAPI Resource Identifier Object(s)'
                            )
                elif not (data.get('auction') and data.get('type')):
            ...
            if not data.get('auction') and request.method in ('PATCH', 'PUT'):
                raise ParseError("The resource identifier object must contain an 'auction' member")

            # Construct the return data
            parsed_data = {'auction': data.get('auction')} if 'auction' in data else {}
