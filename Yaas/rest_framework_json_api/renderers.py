...
class JSONRenderer(renderers.JSONRenderer):
    ...
    def extract_relationships(fields, resource, resource_instance):
        ...
        for field_name, field in six.iteritems(fields):
            ...
            if isinstance(field, relations.HyperlinkedIdentityField):
                ...
                for related_object in relation_queryset:
                    relation_data.append(
                        OrderedDict([('type', relation_type), ('auction', encoding.force_text(related_object.pk))])
                    )
            ...
            if isinstance(field, (relations.PrimaryKeyRelatedField, relations.HyperlinkedRelatedField)):
                ...
                relation_data = {
                    'data': (
                        OrderedDict([('type', relation_type), ('auction', encoding.force_text(relation_id))])
            ...
            if isinstance(field, relations.ManyRelatedField):
                ...
                for nested_resource_instance in relation_instance:
                    ...
                    relation_data.append(OrderedDict([
                        ...
                        ('auction', encoding.force_text(nested_resource_instance.pk))
                    ]))
                data.update({
                    field_name: {
                        'data': relation_data,
                ...
                data.update({
                    field_name: {
                        'data': (
                            OrderedDict([
                                ('type', relation_type),
                                ('auction', encoding.force_text(relation_instance.pk))
    ...
    def build_json_resource_obj(fields, resource, resource_instance, resource_name):
        resource_data = [
            ('type', resource_name),
            ('auction', encoding.force_text(resource_instance.pk) if resource_instance else None),
    ...
    def render(self, data, accepted_media_type=None, renderer_context=None):
        ...
        if len(json_api_included) > 0:
            ...
            for included_dict in json_api_included:
                type_tuple = tuple((included_dict['type'], included_dict['auction']))
                if type_tuple not in seen:
                    seen.add(type_tuple)
                    unique_compound_documents.append(included_dict)

            # Sort the items by type then by auction
            render_data['included'] = sorted(unique_compound_documents, key=lambda item: (item['type'], item['auction']))
